#!/usr/bin/env python3
"""
nac.py — Outil de gestion brownfield Network-as-Code pour Cisco ACI.

Capture une fabric ACI EXISTANTE en YAML NaC (module netascode/nac-aci),
la synchronise avec Terraform, et valide le tout — sans jamais l'ecraser.

Le mapping APIC<->YAML est DERIVE automatiquement du bloc `content {}` des
sous-modules Terraform (source de verite, versionnee) : 100% des attributs,
zero mapping ecrit a la main.

Outil de SYNCHRO pur : fabric <-> data/*.nac.yaml <-> Terraform.
Les outils de test/dev (audit, coverage, selftest, comparaison) sont dans
test_nac.py (séparé) — nac.py ne fait QUE sa job de synchro.

Sous-commandes
--------------
  capture     Aspire la fabric -> data/*.nac.yaml (LECTURE SEULE, TOUS attributs)
  validate    nac-validate du dossier data/
  plan        terraform plan (apercu, ne modifie rien)
  sync        terraform apply (adoption ; garde-fou anti-destruction)
  bootstrap   capture + validate + plan (collecte brownfield complete)

Authentification : lue depuis le bloc `provider "aci"` de main.tf
(surchargeable par les variables d'env APIC_URL / APIC_USER / APIC_PWD).

Usage : python tools/nac.py <sous-commande> [options]
"""
...
def load_creds() -> tuple[str, str, str]:
    ...
    if not all((url, user, pwd)):
        sys.exit("ERREUR : credentials introuvables (provider \"aci\" dans main.tf "
                 "ou variables APIC_URL/APIC_USER/APIC_PWD).")
    return url, user, pwd
...
def capture_tenants(apic: Apic, warnings: list):
    ...
    for b in apic.get_class("fvBD"):
        ...
        vrf = bd_vrf.get(b["dn"])
        if not vrf:
            warnings.append(f"BD '{tn(b['dn'])}/{b['name']}' sans VRF -> ignore")
            continue
    ...
    for l in apic.get_class("l3extOut"):
        ...
        if not (vrf and dom):
            warnings.append(f"L3Out '{tn(l['dn'])}/{l['name']}' sans VRF/domaine -> ignore")
            continue
    ...
    for cls, key in (("mgmtRsOoBStNode", "oob"), ("mgmtRsInBStNode", "inb")):
        for r in apic.get_class(cls):
            ...
            if nid in registered:
                warnings.append(f"adresse mgmt statique du noeud ENREGISTRE {nid} "
                                "non capturee (exigerait role leaf/spine -> node_registration)")
                continue
...
def cmd_capture(args):
    ...
    ver = apic.login()
    log.info("Connecte a l'APIC %s (v%s) — LECTURE SEULE.", apic.url, ver)
    warnings = []
    ...
    with open(os.path.join(DATA_DIR, "modules.nac.yaml"), "w") as f:
        f.write("# Modules desactives : objets a secret non capturables (cle requise).\n"
                "# Genere par tools/nac.py.\n---\n"
    ...
    log.info("  %s : %d tenant(s) applicatifs", p2, napp)
    for t in tns:
    ...
    if warnings:
        log.warning("  ⚠️  %d objet(s) incomplet(s) ignore(s):", len(warnings))
        for w in warnings:
            log.warning("     - %s", w)
    return 0
...
def cmd_sync(args):
    ...
    if ndes and not args.force:
        log.error("ABANDON : le plan DÉTRUIRAIT %d objet(s). Vérifiez que data/ reflète la fabric "
                  "(lancez `nac.py capture`). Utilisez --force pour passer outre.", ndes)
        return 1
    if not args.yes:
        ans = input("terraform apply (adoption dans le state). Continuer ? [y/N] ")
...
def main(argv=None):
    ...
    sub.add_parser("capture", help="aspire la fabric -> data/ (lecture seule)")
    sub.add_parser("validate", help="nac-validate data/")
    sub.add_parser("plan", help="terraform plan (apercu)")
